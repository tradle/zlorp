...
Peer.prototype._keepAlive = function() {
  ...
  if (this._destroyed || this._kaIntervalId || !this.client) return

  // we end up encrypting ACK every time, wasteful
  this._kaIntervalId = setInterval(keepAlive, KEEP_ALIVE_INTERVAL)
...
Peer.prototype._destroy = function(cb) {
  ...
  clearInterval(this._kaIntervalId)
